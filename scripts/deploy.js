...
async function main() {
  ...
  const NFTWorldsPlayersV1Factory = await ethers.getContractFactory('NFT_Worlds_Players_V1', polygonWallet);

  // Deploy Polygon
  const polygonPlayersContract = await NFTWorldsPlayersV1Factory.deploy(
    FORWARDER_ADDRESS,
    IPFS_GATEWAY,
  );
